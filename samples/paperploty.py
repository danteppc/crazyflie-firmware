...
mymodel1 = np.poly1d(np.polyfit(t1, y1, 3))
...
mymodel2 = np.poly1d(np.polyfit(t2, y2, 3))
myline2 = np.linspace(min(t2), max(t2), 10)

print(mymodel2)

#plt.plot(t1, x2, alpha=0.1)
plt.plot(myline2, mymodel2(myline2), marker = 'x', c='red')


mymodel3 = np.poly1d(np.polyfit(t3, y3, 3))
...
plt.legend(['no-spoofing regression for y-estimates', 'spoofing regression for y-estimates', 'spoofing regression for y-estimates under MI-TESLA'])

plt.savefig('regy.pdf')
